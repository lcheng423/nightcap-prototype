...
const boxStyle = {
  ...
  width: 124,
  height: 124,
  padding: "15px 44px 8px 44px",
  flexDirection: "column",
  justifyContent: "center",
  alignItems: "center",
  gap: 6,
  borderRadius: 28,
  background: "rgba(247, 240, 225, 0.50)",
  textDecoration: "none",
  cursor: "pointer",
  border: "none",
...
function useCountUp(target, durationMs = SCORE_DURATION_MS) {
...
function ScoreBox({ slug, Icon, target, label, iconScale, iconBoxSize = 20 }) {
  ...
  const iconBox = iconBoxSize === 20 ? iconBoxStyle : { ...iconBoxStyle, width: iconBoxSize, height: iconBoxSize };
  return (
    <Link
      href={`/score/${slug}`}
      style={boxStyle}
    ...
    >
      <div style={iconBox}>
        <span style={{ display: "flex", alignItems: "center", justifyContent: "center", transform: `scale(${iconScale})` }}>
          <Icon />
        </span>
      </div>
...
export default function HomeScoresRow() {
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      {SCORES.map(({ slug, Icon, target, label, iconScale, iconBoxSize }) => (
        <ScoreBox key={slug} slug={slug} Icon={Icon} target={target} label={label} iconScale={iconScale} iconBoxSize={iconBoxSize} />
      ))}
    </div>
  );
}
